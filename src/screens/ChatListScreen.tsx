      ...
      <main className="max-w-lg mx-auto p-4 flex flex-col gap-6">
        ...
        <section>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-bold text-foreground">Diretas</h2>
          </div>
          <div className="flex flex-col gap-1">
            {directRooms.map((room, i) => {
              const initials = room.name.split(' ').map(n => n[0]).join('').slice(0, 2);
              return (
                <motion.div key={room.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: (clubRooms.length + i) * 0.04 }}>
                  <Link
                    to={`/chat/${room.id}`}
                    className="flex items-center gap-3 p-3 rounded-[var(--radius)] bg-card hover:bg-muted/50 transition-colors"
                    style={{ boxShadow: 'var(--shadow-card)' }}
