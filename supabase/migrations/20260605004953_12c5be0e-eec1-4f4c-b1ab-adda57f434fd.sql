...
ADD COLUMN IF NOT EXISTS developer boolean NOT NULL DEFAULT false;

CREATE OR REPLACE FUNCTION public.is_valid_dev_setup_password(_password text)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT md5(COALESCE(_password, '') || 'pi_club_dev_setup_v1') = '5078fa308a6eff057540d013d2452c49';
$$;
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF NEW.developer IS TRUE AND NOT public.has_role(auth.uid(), 'developer'::app_role) THEN
...
DROP TRIGGER IF EXISTS prevent_manual_developer_profile_changes ON public.profiles;
CREATE TRIGGER prevent_manual_developer_profile_changes
BEFORE INSERT OR UPDATE ON public.profiles
FOR EACH ROW
EXECUTE FUNCTION public.prevent_manual_developer_profile_changes();